...
from python_selenium_course_1.src.pages.locators.CartPageLocator import CartPageLocator
...
class CartPage(CartPageLocator):
    ...
    def get_all_product_names_in_cart(self):
        product_name_elements = self.SeleniumExtended.wait_and_get_elements(self.PRODUCT_NAMES_IN_CART)
        product_names = [i.text for i in product_name_elements]
        return product_names

    def input_coupon(self, coupon_code):
        self.SeleniumExtended.wait_and_input_text(self.COUPON_FIELD, coupon_code)

    def click_apply_coupon(self):
        self.SeleniumExtended.wait_and_click(self.APPLY_COUPON_BTN)

    def get_displayed_message(self):
        txt = self.SeleniumExtended.wait_and_get_text(self.CART_PAGE_MESSAGE)
        return txt
