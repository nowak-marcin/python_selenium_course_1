...
class Bank:
    ...
    def calculate_repaid_amount(self):
        repaid_amount = (self.loan_amount * Bank.spread) + self.loan_amount
        print(f'Klient: {self.customer_name}, calkowita kwota do splaty wynosi: {repaid_amount}.')

    def calculate_repaid_amount_with_penalty(self, penalty_percent):
        penalty = penalty_percent * self.loan_amount
        repaid_amount = (self.loan_amount * Bank.spread) + self.loan_amount + penalty
        print(f'Klient: {self.customer_name}, calkowita kwota do splaty wynosi: {repaid_amount}.')


customer1 = Bank('janusz kowal', 2000)
